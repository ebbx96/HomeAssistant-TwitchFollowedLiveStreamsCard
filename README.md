...
| `streams_vip` | `list` | `EMPTY` | Will show a `✭` in front of the user name of the streamer to mark it as an VIP/favorite. |
...
2. If the Integration isn't working check the properties `global_credentials_access_token` and `global_credentials_client_id` to be correctly set matching the [App](https://github.com/stefmde/HomeAssistant-TwitchFollowedLiveStreamsCard/tree/main#twitch-add-dev-app) you created.
